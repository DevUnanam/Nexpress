...
class RegisterView(CreateView):
    """
    User registration view with role selection and email verification
    """
    ...
    def get(self, request, token):
        try:
            ...
            if not user.is_verification_token_valid():
                messages.error(
                    request,
                    'This verification link has expired. Please contact support to resend the verification email.'
                )
            ...
            messages.error(
                request,
                'Invalid verification link. Please check the link or contact support.'
            )
            return redirect('login')
